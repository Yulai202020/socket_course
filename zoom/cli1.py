...
receiver = AudioReceiver(local_ip_address, 4444)
...
def start_camera_stream():
    camera_client = CameraClient(text_target_ip.get(1.0, "end-1c"), 5555)
    t3 = threading.Thread(target=camera_client.start_stream)
    t3.start()

def start_screen_sharing():
    screen_client = ScreenShareClient(text_target_ip.get(1.0, "end-1c"), 5555)
